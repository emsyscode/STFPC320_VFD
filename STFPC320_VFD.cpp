...
// Note: (Write: 0x52 and Read: 0x53) Board with STFPC320 !!!!
int flag = 0;
int ack;
...
unsigned char arrNumbers[10] = {0x3F, 0x06, 0x5B, 0x4F,0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F}; // This is a table to draw the 7 segments digits
...
unsigned char digitB=0;
 ...
 int flagWriteMM =0;
 int flagWriteHH =0;
 int flagDE25=0;  // Led red stay near of connector BE1
/******************* RTC *************************/
...
// Here we do the configuration of format HH:MM:SS or HH:MM and segments position
    ...
    send_byte(0xAB); readAcknowledge();
    send_byte(0xEE); readAcknowledge();
    send_byte(0x60); noAcknowledge(); 
    // Byte 7 to set the DSP //Important the bit 6(0x40) at value 1 define a digit between hh&mm and mm&ss. 
    // Bit 5(0x60) to 1 is HH:MM:SS, 0x20 set 12H with AM/PM or 0x03 to show AM/PM(note you need the display of 14sgments to draw he "M"
    stp();
}
void setTimer(void){
  /*************************  Set of Sec, Mins, Hours, Day ...**********************************/
            ...
            readAcknowledge();
            send_byte(0b11110000);  // Set day, The 4 bits on left is from RS3-RS0 set of pulse to external. 
            noAcknowledge();
            stp();
}
 /*************************  Set of Sec, Mins, Hors, Day Alarm **********************************/
void setAlarms(void){
  // Can use hte same mode to set Alarms... I'm not implement it, but the idea is the same of set Clock.
            start();
            send_byte(addressWR);  // slave with write active 
            readAcknowledge();
            send_byte(0b01000000);   //  The bit B2 is 1 to fix address and 0 to auto-increment address
            ...
            send_byte(addressWR);  // slave with write active 
            readAcknowledge();
            send_byte(0b11001100);   // 0x0C address register of RTC, see datasheet page: 26/78; 6.5 Register table for RTC Table:4
            readAcknowledge();
...
void begConf(void){
      //initSTFPC320(); I'm not using this init, now!
